...
def parse_et_to_utc(dt_str: str) -> datetime:
    """
    dt_str format: 'YYYY-MM-DD HH:MM' in Eastern time
    """
...
def db_insert_messages(conn: sqlite3.Connection, symbol: str, msgs: list[dict]) -> int:
    ...
    for msg in msgs:
        ...
        if created_at_utc >= END_UTC:
            # too new (after end of window)
            continue
...
def main():
    ...
    parser.add_argument("symbols", nargs="*", help="Symbols. Defaults: SMX.")
    ...
    global START_UTC, END_UTC
    START_UTC = parse_et_to_utc(args.start_et)
    END_UTC = parse_et_to_utc(args.end_et)

    symbols = [s.upper() for s in (args.symbols if args.symbols else ["SMX"])]
    ...
    try:
        while True:
            ...
            if now_utc >= END_UTC:
                print(f"Reached END_ET cutoff ({END_UTC.astimezone(ET)}). Stopping.")
                break
